...
class UpdateMagentoSpecialPricesJobTest extends TestCase
{
    public function test_it_calls_action(): void
    {
        $this->mock(UpdatesMagentoSpecialPrice::class, function (MockInterface $mock) {
            $mock->shouldReceive('update')->once();
        });

        UpdateMagentoSpecialPricesJob::dispatchSync(new PriceData('::sku::', collect()));
    }

    public function test_queue_attributes(): void
    {
        $job = new UpdateMagentoSpecialPricesJob(new PriceData('::sku::', collect()));
